...
{
    [DisplayName( "SSO Initiator" )]
    [Category( "BEMA Services > Security" )]
    [Description( "Automatically Authenticates Person against selected Authentication Provider" )]
    [CustomDropdownListField( "Active Authentication Provider", "The Authentication Provider we are going to use for SSO",
        @"SELECT [FriendlyName] AS [Text], [Name] AS [Value] FROM [EntityType] WHERE [Name] IN (
            'Rock.Security.ExternalAuthentication.Google',
            'com.bemaservices.Security.SSO.Authenticators.Office365',
            'com.bemaservices.Security.SSO.Authenticators.Okta',
            'Rock.Security.Authentication.Auth0.Auth0Authentication',
            'com.bemaservices.Security.SSO.Authenticators.Vision2'
        )", true, "com.bemaservices.Security.SSO.Authenticators.Office365" )]
    ...
    public partial class SSOInitiator : Rock.Web.UI.RockBlock
    {
        protected override void OnLoad( EventArgs e )
        {
            if ( !Request.IsAuthenticated )
            {
                ...
                var selectedAuthProvider = GetAttributeValue( "ActiveAuthenticationProvider" );

                if( !string.IsNullOrEmpty( selectedAuthProvider ))
                {
                    var authenticationProvider = AuthenticationContainer.GetComponent( selectedAuthProvider );
                    ...
                    if ( authenticationProvider != null )
                    {
                        ...
                        if ( !IsPostBack && authenticationProvider.IsReturningFromAuthentication( Request ) )
                        {
                            ...
                            else
                            {
                                ...
                                var excceptionText = string.Format( "SSO - failed to login user: {0}", userName );
                        ...
                        else
                        {
                            if ( !IsPostBack && !string.IsNullOrEmpty( GetAttributeValue( "AllowedNetwork" ) ) )
                            {
                                ...
                                {
                                    // Logging exception to make troubleshooting easier
                                    var excceptionText = string.Format( "SSO - IP Address not in CIDR Network: {0}", userIPAddress );
                                    ExceptionLogService.LogException( new Exception( excceptionText ) );
                            ...
                            {
                                // Sending to Authentication Provider specific page to begin authorization
                                Response.Redirect( authorizationLink.AbsoluteUri );
                                Context.ApplicationInstance.CompleteRequest();
                            }
                        }
